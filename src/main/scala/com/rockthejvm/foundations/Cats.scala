...
object Cats {
  ...
  val mappedList = listFunctor.map(List(1, 2, 3))(x => x + 1)
  ...
  val aSimpleListV2 = 42.pure[List] // same as aSimpleList
  ...
  def crossProductV3[F[_] : Monad, A, B](containerA: F[A], containerB: F[B]): F[(A, B)] =
    for {
      a <- containerA
      b <- containerB
    } yield (a, b)
  ...
  def main(args: Array[String]): Unit = {
    println(aSimpleList)
    println(crossProduct(List(1, 2, 3), List("a", "a", "c")))
  }
}
